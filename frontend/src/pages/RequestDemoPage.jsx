...
export default function RequestDemoPage() {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    // TODO: wire to backend /api/demo-requests when endpoint exists
    setSubmitted(true);
  }
      ...
      <div className="max-w-2xl mx-auto px-4 pt-32 pb-20">
        ...
        {submitted ? (
          <div className="text-center py-16">
            ...
            <p className="text-slate-400">We'll reach out within 1 business day to schedule your demo.</p>
          </div>
        ) : (
          <>
            <h1 className="text-3xl font-bold text-white mb-2">Request a Demo</h1>
            <p className="text-slate-400 mb-8">Tell us about your school and we'll set up a personalised walkthrough.</p>
              ...
              {[
                ...
                <div key={f.name}>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/15 text-white placeholder-slate-500 text-sm focus:outline-none focus:border-[#0ABAB5]/60 focus:ring-1 focus:ring-[#0ABAB5]/30 transition-all"
                  />
                </div>
              ))}

              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1">Anything you'd like us to know?</label>
                <textarea
                  name="message"
                  value={form.message}
                  onChange={handleChange}
                  ...
                  className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/15 text-white placeholder-slate-500 text-sm focus:outline-none focus:border-[#0ABAB5]/60 focus:ring-1 focus:ring-[#0ABAB5]/30 transition-all resize-none"
                />
              </div>
