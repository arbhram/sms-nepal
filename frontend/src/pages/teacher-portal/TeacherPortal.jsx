...
import { BookOpen, Users, CalendarCheck, Award, LogOut, Menu, X, KeyRound } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 flex">
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-white border-r flex flex-col transition-transform duration-200 ${open ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static lg:flex`}>
        <div className="p-5 border-b flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-sm">T</div>
          <div>
            <p className="font-display font-bold text-slate-900 text-sm leading-tight">Teacher Portal</p>
        ...
        {profile && (
          <div className="p-4 border-b">
            ...
            <p className="text-xs text-slate-500">{profile.subject}</p>
            <p className="text-xs text-slate-400 mt-0.5">{profile.teacherId}</p>
          </div>
        )}
        ...
        <nav className="flex-1 p-3 space-y-1">
          {NAV.map(({ to, icon: Icon, label }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50'}`
              }
            >
              <Icon size={16} /> {label}
            </NavLink>
